...
private Q_SLOTS:
    ...
    void addToplevel();
    void addMappedToplevel();
    void restoreToplevel();
    void restoreMappedToplevel();
};
...
void TestXdgSession::addToplevel()
{
    QString sessionId;
    ...
    // If the toplevel is added to the session for the first time, we should receive a created event, but no restored event.
    {
        ...
        sessionId = sessionCreatedSpy.last().at(0).toString();
    ...
    // If the toplevel is added to the session again, no created and restored events should be sent.
    {
        std::unique_ptr<KWayland::Client::Surface> surface(Test::createSurface());
        std::unique_ptr<Test::XdgToplevel> shellSurface(Test::createXdgToplevelSurface(surface.get(), Test::CreationSetup::CreateOnly));
        std::unique_ptr<Test::XdgSessionV1> session(Test::createXdgSessionV1(Test::XdgSessionManagerV1::reason_launch, sessionId));
        std::unique_ptr<Test::XdgToplevelSessionV1> toplevelSession(session->add(shellSurface.get(), QStringLiteral("foo")));
...
void TestXdgSession::addMappedToplevel()
{
...
void TestXdgSession::restoreToplevel()
{
    QString sessionId;
...
void TestXdgSession::restoreMappedToplevel()
{
