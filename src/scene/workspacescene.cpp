...
static std::optional<SurfaceItem *> findOverlayCandidate(SceneView *delegate, Item *item, QRegion &occupied)
{
    if (!item || !item->isVisible()) {
        return nullptr;
    }
    if (item->hasEffects()) {
        // we don't know what the effect might be doing, cancel the search
        // FIXME add more information on the item than just "has effects"
        // as blur for example should not cancel the search for candidates
        return std::nullopt;
    }
