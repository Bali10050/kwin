...
void StrokeEffect::reconfigure(ReconfigureFlags)
{
    ...
    effects->registerStrokeShortcut(rightwardPoints, m_actions.back().get());

    m_actions.push_back(std::make_unique<QAction>());
    connect(m_actions.back().get(), &QAction::triggered, this, [] {
        qDebug() << "STROKE LEFT!";
    });
    effects->registerStrokeShortcut(leftwardPoints, m_actions.back().get());
}
...
void StrokeEffect::strokeGestureBegin(const QList<QPointF> &points, std::chrono::microseconds time)
{
    activate();
    Q_EMIT strokeStarted(points.first());
}

void StrokeEffect::strokeGestureUpdate(const QList<QPointF> &points, std::chrono::microseconds time)
{
    Q_EMIT strokePointAdded(points.last());
}
